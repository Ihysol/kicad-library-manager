...
* Or run the CLI:
  ...
  ```
* ZIPs are read from the `library_manager/library_input` folder (or the folder you pick inside the GUI).

### Workflow

1. Drop or select `.zip` files into `library_manager/library_input` (or choose a folder in the GUI).
...
* **library_manager/library_input/** â€“ Folder for `.zip` files or component definitions to be imported (default source).
* **library_manager/library_output/** â€“ Destination for exported ZIPs.
* **library_manager/dru_templates/** â€“ DRC templates consumed by the DRC tab.
